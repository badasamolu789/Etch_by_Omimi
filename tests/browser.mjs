...
try {
    ...
    assert.equal(await page.locator('#productsGrid a').first().getAttribute('href'),'/product-detail?slug=real-slug');
    ...
    await page.waitForFunction(()=>window.__confirmation===null);
    await page.evaluate(()=>{window.__confirmation='waiting';EtchDialog.confirm('Delete this test record?').then(value=>window.__confirmation=value);});
    await page.getByRole('button',{name:'Delete',exact:true}).click();
    await page.waitForFunction(()=>window.__confirmation===true);
    await page.evaluate(()=>{window.__answer='waiting';EtchDialog.prompt('Expiry','48',{type:'number',min:1,max:168}).then(value=>window.__answer=value);});
    await page.getByRole('dialog').waitFor();
    await page.locator('dialog input').fill('72');
    await page.getByRole('button',{name:'Continue',exact:true}).click();
    await page.waitForFunction(()=>window.__answer==='72');
